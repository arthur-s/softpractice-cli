...
func printCommandHelp(ctx context.Context, output io.Writer, command string) error {
	...
	case "submission":
		russian, english = "Использование: softpractice submission show [--id ID] [--format text|json|json-v2]\n\nПоказывает безопасный статус и результат своей отправки. json-v2 сохраняет полный безопасный результат для автоматизации.", "Usage: softpractice submission show [--id ID] [--format text|json|json-v2]\n\nShows the safe state and result of your submission. json-v2 preserves the full safe result for automation."
	case "submit":
		russian, english = "Использование: softpractice submit [--yes]\n\nОтправляет чистый Git commit текущего урока на проверку.", "Usage: softpractice submit [--yes]\n\nSubmits the clean Git commit for the current lesson."
	case "update":
		russian, english = "Использование: softpractice update\n\nПосле принятия решения добавляет файлы текущего следующего урока в этот же проект.", "Usage: softpractice update\n\nAfter acceptance, adds the files for the current next lesson to this project."
	case "open":
	...
	case "set":
		russian, english = "Использование: softpractice set api-url|web-url|lang VALUE\n\nСохраняет пользовательскую настройку CLI.", "Usage: softpractice set api-url|web-url|lang VALUE\n\nSaves a user-level CLI setting."
	case "config":
